...
async def like(comment_id : int, db : AsyncSession = Depends(get_db)) -> ResponseModel[CommentResponse] : 
    try : 
        response : CommentResponse = await comment_service.like(comment_id=comment_id, db=db)
        return ResponseModel[CommentResponse](
            success=True,
            data=response,
...
async def dislike(comment_id : int, db : AsyncSession = Depends(get_db)) -> ResponseModel[CommentResponse] : 
    try : 
        response : CommentResponse = await comment_service.dislike(comment_id=comment_id, db=db)
        return ResponseModel[CommentResponse](
            success=True,
            data=response,
